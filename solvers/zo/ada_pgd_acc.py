...
class AdaPGDVR(object):
    ...
    def update(self):
        ...
        self.md(self.t*g)
        tau=self.t/self.a_t
        self.z=tau*self.x+(1-tau)*self.z
        return self.z
    ...
    def md(self,g):
        ...
        v_sgn = np.sign(z)
        v_val = np.maximum(eta_t_m_1*np.abs(z)-(self.t+1)*self.l1,0.0)/(eta_t_m_1+(self.t+1)*self.l2)
        v = v_sgn * v_val
        v = np.clip(v, self.lower, self.upper)
        self.eta+=((lina.norm((v-self.z).flatten(),ord=2)*eta_t_m_1)**2)
        eta_t = np.maximum((self.eta)**self.p,1.0)
        self.x=(1-eta_t_m_1/eta_t)*self.x+eta_t_m_1/eta_t*v

def fmin(func,x0, upper,lower,l1=1.0,l2=1.0, maxfev=50, batch=10, callback=None, epoch_size=10,eta=1.0):
    delta=1.0/maxfev/np.sqrt(x0.size)
    b=batch
    func_p=Grad_2p_batch(func,b,delta)
    alg=AdaPGDVR(func=func,func_p=func_p,x0=x0,upper=upper,lower=lower,l1=l1,l2=l2,eta=eta)
    fev=1
    y=None
    while fev <= maxfev:
        y=alg.update()
